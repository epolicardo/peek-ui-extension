...
class FavoriteTreeItem extends vscode.TreeItem {
    constructor(
        ...
        if (contextValue === 'favoriteEnvironment') {
            ...
            if (favoriteItem) {
                this.command = {
                    command: 'horgen.peek-ui.openFavorite',
                    title: 'Open Favorite',
...
export class FavoritesProvider implements vscode.TreeDataProvider<FavoriteTreeItem> {
    ...
    async getChildren(element?: FavoriteTreeItem): Promise<FavoriteTreeItem[]> {
        if (!element) {
            ...
            const favorites = this.context.globalState.get<IFavoriteItem[]>('horgen.peek-ui.favorites', [])
            ...
            const envNodesPromises = Array.from(grouped.entries()).map(async ([sbName, items]) => {
                // Get alias if exists
                const alias = await this.context.globalState.get<string>(`horgen.peek-ui.alias.${sbName}`)
    ...
    async addToFavorites(item: QueueItem | TopicItem | SubscriptionItem): Promise<void> {
        ...
        await this.context.globalState.update('horgen.peek-ui.favorites', favorites)
    ...
    async removeFromFavorites(item: FavoriteTreeItem): Promise<void> {
        ...
        await this.context.globalState.update('horgen.peek-ui.favorites', filtered)
    ...
    async openFavorite(favorite: IFavoriteItem): Promise<void> {
        try {
            ...
            const connectionString = await this.context.secrets.get(`horgen.peek-ui.connection.${favorite.serviceBusName}`)
